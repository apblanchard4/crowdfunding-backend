...
exports.lambdaHandler = async (event, context, callback) => {
   ...
   // ready to go for CORS. To make this a completed HTTP response, you only need to add a statusCode and a body.
    ...
    let CheckProjectExistence = (name) => {
        return new Promise((resolve, reject) => {
            pool.query("SELECT * FROM Project WHERE idProject=?", [name], (error, rows) => {
    ...
    let CheckProjectValid = (name) => {
        return new Promise((resolve, reject) => {
            pool.query("SELECT * FROM Project WHERE idProject=? and isActive =? and isVisible = ?", [name,true,true], (error, rows) => {
                if (error) { return reject(error); }
                if ((rows) && (rows.length == 1)) {
                    return resolve(true);   // TRUE if does exist
                } else { 
    ...
    let getPledge = (name) => {
        return new Promise((resolve, reject) => {
            pool.query("SELECT * FROM Pledges WHERE idPledge=?",[name], (error, rows) => {
                if (error) { return reject(error); }
                if ((rows) && (rows.length == 1)) {
                    //console.log("rows!!!!!!!!!!!!!: " ); 
                    console.log(rows); 
    ...
    try {
        ...
        console.log("E1")
        var exists = await CheckProjectExistence(info.projectID);
        const valid = await CheckProjectValid(info.projectID);
        const pledgeExsits = await CheckPledgeExistence(info.pledgeID);
        exists = exists && valid && pledgeExsits;
            if (exists) {
                console.log("E2")
                let projectInfo = await getPledge(info.pledgeID);
                console.log("E3")
                ...
                response.statusCode = 200;
                 ...
                 response.error = "Couldn't find any projects named " + info.pledgeID;
            }
