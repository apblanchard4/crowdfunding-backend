...
exports.lambdaHandler = async (event, context, callback) => {
   ...
   // ready to go for CORS. To make this a completed HTTP response, you only need to add a statusCode and a body.
    ...
    let CheckNameExistence = (name) => {
        return new Promise((resolve, reject) => {
            pool.query("SELECT * FROM Supporters WHERE supporterName=?", [name], (error, rows) => {
    ...
    let InsertNameValue = (name, email) => {
        return new Promise((resolve, reject) => {
            ...
            pool.query("INSERT INTO Supporters (supporterName,supporterEmail) VALUES(?,?)", [name, email], (error, rows) => {
                if (error) { return reject(error); }
                if ((rows) && (rows.length == 1)) {
    ...
    try {
        ...
        if (exists) {
            ...
            response.error = "This supporter is already exist";
        } else {
            ...
            if (inserted) {
                ...
                console.log("Success, created supporter: " + info.name);
            } else {
                 response.statusCode = 400;
