...
exports.lambdaHandler = async (event, context, callback) => {
   ...
   // ready to go for CORS. To make this a completed HTTP response, you only need to add a statusCode and a body.
    ...
    let getProjects = (designerID) => {
        ...
        return new Promise((resolve, reject) => {
            pool.query("SELECT * FROM Project WHERE idDesigner =?", [designer], (error, rows) => {
                if (error) { return reject(error); }
                if (rows) {
    ...
    try {
        ...
        if (designerID != false) {
             const projects = await getProjects(designerID); // Projects parameter isn't actually passed into getProjects; is it necessary?
            ...
            response.statusCode = 400;
            response.error = "There are no projects for this designer";
        }
